...
for (const [width, height] of [[1440, 900], [390, 844]]) {
  ...
  const advancedView = page.getByRole("button", { name: "View Advanced screenshot", exact: true });
  await advancedView.scrollIntoViewIfNeeded();
  const advancedSize = await advancedView.boundingBox();
  if (!advancedSize || advancedSize.height < 44) failures.push(`${width}: Advanced screenshot action is too small`);
  await advancedView.focus();
  await page.keyboard.press("Enter");
  await page.locator('#advanced-screenshot-dialog[open]').waitFor({ state: "visible" });
  await page.keyboard.press("Escape");
  if (!await advancedView.evaluate(el => document.activeElement === el)) failures.push(`${width}: Advanced action lost return focus`);
  const standardView = page.locator('.studio-standard-capture summary');
  await standardView.scrollIntoViewIfNeeded();
  const standardSize = await standardView.boundingBox();
  if (!standardSize || standardSize.height < 44) failures.push(`${width}: Standard screenshot action is too small`);
  await standardView.focus();
  await page.keyboard.press("Enter");
  if (!await standardView.evaluate(el => el.parentElement.open && el.innerText.includes('Hide Standard screenshot'))) failures.push(`${width}: Standard screenshot did not expose its open state`);
  await page.keyboard.press("Enter");
  if (await standardView.evaluate(el => el.parentElement.open)) failures.push(`${width}: Standard screenshot did not close by keyboard`);
