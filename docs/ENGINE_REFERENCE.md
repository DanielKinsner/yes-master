...
3. **7-band tonal EQ** — the musical EQ. Default frequencies: **80 Hz** (sub) · **200 Hz** (low shelf) · **400 Hz** (low-mid) · **1.5 kHz** (mid) · **3.5 kHz** (high-mid) · **6 kHz** (air shelf) · **12 kHz** (sparkle shelf). Since 2026-08-18 each band's frequency is a setting (`eq_bands`, `EqBandFrequencies` in `types.rs`) — dragging a Visual EQ node sideways moves it within a per-band window (sub 30–150 · low 100–350 · low-mid 250–800 · mid 800–3k · high-mid 2k–5.5k · high 4k–10k · sparkle 8k–16k) so bands cannot cross; Q and shelf slope stay fixed, and the defaults are byte-identical to the old constants. Each band = `preset move (scaled by Intensity, possibly trimmed by the guardrail) + your manual move`.
