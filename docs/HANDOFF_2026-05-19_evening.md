...
**Status:** Slice merged to master at `b424b36` via fast-forward on 2026-05-19. Safety tag `pre-eq-7-band-2026-05-19` retained as rollback anchor.
...
- In-flight CSS tweaks stashed pre-merge and restored to the branch checkout after.
...
4. **Cross-platform SHA portability.** Pending empirical Windows verification of Commit 0 SHAs. If divergent, OS-gated constants in-slice; portable tanh as its own DSP-output-changing slice.
...
*Handoff for next session. Slice merged to master via fast-forward at `b424b36` on 2026-05-19.*
