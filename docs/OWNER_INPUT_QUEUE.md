...
the September 5 follow-up choices are all answered and implemented locally.
See checkpoint 9 in the follow-up plan for verification and remaining limits.
...
| Date | Unit | Question | Conservative default in place | What changes when answered |
|---|---|---|---|---|
| ~~2026-09-05~~ | ~~Listening follow-up 2A/2B~~ | ~~Import readiness/loading experience~~ **ANSWERED:** make each completed track usable, show progress for remaining tracks, and prioritize playback over background analysis. See the loading decision in `docs/OPEN_THREADS_AND_DECISIONS.md`. | Implemented locally; per-track native readiness/timing and verification limits are in checkpoint 9. | Publish per-track results and backend profiles incrementally; measure first-ready/batch timing and playback impact. |
| ~~2026-09-05~~ | ~~Listening follow-up 2~~ | ~~Start Preview LUFS automatically on import when the selected mode already requests it?~~ **ANSWERED:** yes, for the selected track when ready; show "Measuring," cancel obsolete work on track switches, and protect playback. Other imported tracks wait until selected. See `docs/OPEN_THREADS_AND_DECISIONS.md`. | Implemented with selected-track cancellation, one shared heavy-preview permit and unchanged enabled state/defaults; see checkpoint 9. | Implement bounded selected-track premeasurement with truthful pending state and source/settings cancellation. |
...
| ~~2026-09-01~~ | ~~Ship review S2~~ | ~~Custom domain?~~ **RESOLVED:** September 5 owner decision already names `yesdsp.com` as the production marketing site. September 21 Vercel/API and HTTP checks verify it is attached and redirects to `www.yesdsp.com`. | Canonical and all share URLs are corrected together in the website follow-up. | Deploy after the publication checkpoint; actual chat-paste rendering remains a separate check. No domain purchase or repeated owner choice is needed. |
