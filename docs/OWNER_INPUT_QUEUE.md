...
| 2026-08-31 | Audit T-03 (for Task 14 site activation) | **Must the HERO CTA itself sit above the fold on short phones, or is the fixed nav CTA the acquisition guarantee while the hero CTA stays below the introductory copy?** The landing test suite historically claimed "fully visible" while measuring only horizontal clipping; it now gates the nav CTA on both axes and the hero CTA horizontally, and records hero fold containment per viewport in `summary.json` as evidence for this decision. | Layout unchanged: the fixed nav CTA remains the blocking all-axis acquisition control; the hero CTA may sit below the fold on short phones. No copy reordering. | If hero-above-fold: move the hero CTA group directly after the headline, tighten phone-only gap/padding, promote hero all-axis containment to blocking, and get owner visual approval of the changed mobile order (Task 14 of the 2026-08-31 remediation plan). If copy-first: make the nav/hero distinction permanent in `docs/TESTING.md`. |
