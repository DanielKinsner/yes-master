...
| 2026-07-27 | U14→U16 | **Merge the proven release-workflow fix and move the candidate tag?** The `v0.9.1-beta.1` tag run failed its macOS draft build on three latent Release-workflow bugs (empty-but-set `APPLE_SIGNING_IDENTITY` broke ad-hoc codesign; empty-but-set notarization trio forced notarization with blanks; the audit resolved drafts via `releases/tags/<tag>`, which cannot see drafts — so the audit had never actually passed). All three are fixed on branch `fix/u16-release-macos-adhoc-signing` (3 commits, workflow file only) and **proven by dispatch run `30292750295`: preflight + both platform builds + full-asset audit all green.** | Branch stays unmerged (`main` frozen); tag stays at `9317a00`; the incomplete `v0.9.1-beta.1` draft release and the proving-run `manual-*` drafts stay in place, unpublished. | Owner picks: merge the branch + move `v0.9.1-beta.1` to the new tip so the real tag run produces the complete draft (workflow-only diff — no product lane reads it, so U14's evidence stands), or fold into U16's re-tag. Stray draft releases can then be deleted. |
