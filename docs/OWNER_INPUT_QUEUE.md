...
| 2026-08-20 | Review follow-up | **Should Space keep driving play/pause when a checkbox or radio has focus?** The global Space transport (Phase 12.1, DAW-style "Space always plays") deliberately overrides Space on buttons/knobs/selects — those have Enter or arrows. But Space is the *only* keyboard way to toggle a checkbox (e.g. Link stereo), so a keyboard-only user cannot flip it. The 2026-08-20 adversarial review flagged this; the narrow fix is to let Space through on `checkbox`/`radio` inputs only, keeping the DAW feel everywhere else. | Current behavior stands (Space = transport everywhere outside text entry) — it is the owner-requested design and changing it is a feel decision. | One small carve-out in the Space handler (`src/hooks/useTrackMaster.ts`) + a regression test; the shortcut overlay copy stays unchanged. |
| 2026-08-20 | Review follow-up | **Do the single-key shortcuts (A / L / ?) need a disable/remap preference?** WCAG 2.1.4 wants single-character shortcuts to be disableable, remappable, or focus-scoped — mainly a speech-input concern. Single-key transport is the DAW genre norm and the keys already yield to text entry and value controls. | Ship as-is for beta; treat a "disable single-key shortcuts" preference as post-beta polish, not a gate. | A settings toggle (persisted like Volume Match) that suppresses the A/L/? handlers; the `?` overlay would note the setting. |

## Answered

_None yet. When a question is answered, strike the row through here and record
the decision in `docs/OPEN_THREADS_AND_DECISIONS.md`._
