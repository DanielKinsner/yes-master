...
Concrete September 5 examples and the proposed WAV + MP3 mastering-export
scope are now in [the output options](plans/2026-09-05-export-options.md).
Loading readiness and automatic selected-track Preview LUFS were answered in
the follow-up interview and are recorded below;
...
| ~~2026-09-05~~ | ~~Listening follow-up 2~~ | ~~Start Preview LUFS automatically on import when the selected mode already requests it?~~ **ANSWERED:** yes, for the selected track when ready; show "Measuring," cancel obsolete work on track switches, and protect playback. Other imported tracks wait until selected. See `docs/OPEN_THREADS_AND_DECISIONS.md`. | Implementation and resource/lifecycle verification remain pending; enabled state/defaults stay unchanged. | Implement bounded selected-track premeasurement with truthful pending state and source/settings cancellation. |
| ~~2026-09-05~~ | ~~Listening follow-up 5C manifest~~ | ~~Album manifest presentation~~ **ANSWERED:** use `metadata/manifest.json`; explain its purpose in the receipt so users do not receive an unexplained file. See `docs/OPEN_THREADS_AND_DECISIONS.md`. | Implementation remains pending; existing exports stay untouched. | Update paths, links, folder creation and cleanup; explain that supporting export details are not needed to play or share audio. |
| ~~2026-09-05~~ | ~~Listening follow-up 5C naming~~ | ~~Master filename suffix~~ **ANSWERED:** Track `Song_mastered.wav`; Album `01-Song_mastered.wav`, one underscore, numbered in final left-rail order. See `docs/OPEN_THREADS_AND_DECISIONS.md`. | Implementation pending; existing exports retain their names. | Apply new default names with source/prior-output protection. |
| 2026-09-05 | Listening follow-up 6 | **Remaining MP3 scope:** settle Track/Album/Standard availability and quality/defaults. **Already settled:** MP3 is a smaller-file choice in normal mastering export; no separate converter or Original-source selector, no change to 0% processing and no special bypass explanation. See `docs/OPEN_THREADS_AND_DECISIONS.md`. | Existing WAV delivery stays truthful; MP3 is not yet implemented. Earlier conversion-without-mastering interpretation is superseded. | Deliver the agreed MP3 export increment and update product/behavior/help descriptions to match verified functionality. |
