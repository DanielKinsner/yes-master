...
| ID | Claim (as currently published) | Surface | Status | Evidence source | Owning unit |
|---|---|---|---|---|---|
| C-10 | "Download the free beta" → `https://github.com/DanielKinsner/yes-master/releases/latest` | `BetaDownload.tsx` / `release-config.ts` | **Remove (as unconditional)** | **No verified full release exists.** `/releases/latest` does not resolve to an installable beta today, and GitHub's `/latest` cannot see drafts (KTD3, go/no-go §3(ii)). This is the dead-primary-CTA the program exists to prevent. Must become release-state-driven and default to unavailable. | U5 |
| C-11 | "Windows + universal Mac" | `BetaDownload.tsx` | Proved (as intent) → **needs artifact binding** | `release.yml` builds one universal macOS artifact + Windows MSI/NSIS; `release-readiness.test.ts` pins that contract. Binding to *actual downloadable artifacts* is U16/U17. | U5, U16 |
