...
decision-complete plan from the 2026-09-01 whole-product review (owner
decisions D1–D11 locked in-session). Nothing tagged or published (D3).
Commits on `main`, in order:

- **Docs (`c5dec99`):** the owner launch checklist's Mac step uses the macOS
  15+ Gatekeeper path (Privacy & Security → Open Anyway); the beta guide gains
  a display-scaling caveat (D6); the owner queue gains the custom-domain row
  (D1) and closes T-03 copy-first (D10); the nav-CTA (both axes) vs hero-CTA
  (horizontal only) rule is permanent in `docs/TESTING.md`; the July Vera
  ship plan is archived.
- **Landing origin (`c9f1b67`):** canonical, `og:url`, `og:image` and
  `twitter:image` publish from `https://yes-master.vercel.app` — no custom
  domain was bought; a same-origin test guards all four; the signup error
  copy drops the unowned mailbox; "Eight presets" → "Eight styles" (D9).
- **Window fits the screen (`0f67d87` tier 1, `5418039` tier 2 — D4):** the
  main window opens centred at 1920×1080 when the primary monitor's logical
  work area holds it and maximized when it does not; below the 1360×740
  layout floor the webview zooms natively to fit, clamped at 0.8 (the
  owner's revert-not-tune rule is in the commit message). Ten unit tests.
- **Dependabot:** all 17 open PRs closed, none merged, until after the tag;
  the post-launch merge batches are queued in the plan's §9.
- **Tier B polish:** one label for import, "Import audio", at all three
  entry points (`25d3b53`); Settings drops the placeholder Build row — Help
  owns the build stamp (`eb8330b`); render and audio-device failures map to
  plain language (`c953bfa`); both receipt surfaces say when never-overwrite
  saved a render under a `__n` name and name the file that was left alone
  (`06e8a41`); "EQUALIZER (Dynamic)" → "EQUALIZER" (`030f134`, D11a); the
  receipt's mastering-target chip uses short profile names, "Streaming ·
  -14.0 LUFS" (`7d16b06`, D11b).
- **Landing captures** were regenerated by the asset gate each time a captured
  surface changed (four recaptures); the owner's real-session plates are
  untouched and still show the pre-rename rail label until re-plated by hand.
