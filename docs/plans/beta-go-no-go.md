...
| 2026-07-25 | U6 | `PLACEHOLDER_U6` | WSL2 Ubuntu / Node 22, vitest 4.1.9 | source `0.9.0` | `npm test`, `npm run build` | frontend-unit | PASS — 69 files, 697 tests, 3 skipped; build clean. 17 new copy invariants rendered via `renderToStaticMarkup`, so every claim asserted is content a visitor receives with **no JavaScript and no animation** — the "essential content understandable when animations do not run" half of S-A1/S-A2/S-I1. | `src/landing/LandingCopy.test.tsx` |
| 2026-07-25 | U6 | `PLACEHOLDER_U6` | WSL2 Ubuntu / Node 22 | — | **negative controls** (4) | frontend-unit | PASS — restoring "coming after launch", moving `CrossPlatform` back to second, restoring "beta testers keep $29 forever", and restoring "true-peak safe, every time" turn **6** tests red between them, including the positional hierarchy and audience-order assertions. | run logs |
| 2026-07-25 | U6 | `PLACEHOLDER_U6` | WSL2 Ubuntu / Playwright bundled Chromium 149.0.7827.55 | `dist/` from source `0.9.0` | `npm run verify:headless` | **browser-headless** | PASS — landing suite green at all 12 viewports against the rebuilt page (9 required sections, `requiresNavAnchor` added so `#mobile` can exist without a nav slot); app suite 24/24 unchanged. | `test-output/headless/2026-07-25T20-36-42-426Z/` |
