...
> **2026-09-21 — Launch preparation; not yet GO.** The owner requested readiness
...
> Preserve the prior Windows listening approval. Exact candidate packaging/CI,
> Mac real-machine evidence and the public updater/release transaction remain
...
| 2026-09-01 | U15-class hands-on QA (Mac) | `3d9ba6c` | macOS 26.5 Apple Silicon (M-series), locally built packaged `.app` | prospective main build `v0.9.2-beta.1-30-g3d9ba6c` | full hands-on packaged-app pass: every Standard/Advanced control family, album flows, project save/reload, mid-render cancellation, hostile/edge inputs (unicode, empty, malformed, silent, hot, 8 kHz–96 kHz), export integrity via independent SHA-256 | installed-machine | **FAIL — launch blocker found.** Changing only Track Master bit depth or sample rate flipped the delivery profile to Custom *without* capturing the effective profile values first, so a hidden analysis offset became the active loudness target: Streaming −14.0 LUFS → **0.3 LUFS** after selecting 32-bit float (reproduced separately with 96 kHz); a silent stress fixture showed **56.0 LUFS**. A format-only choice silently changed mastering loudness. The defect also exists in the tagged `c750da6` bytes, so the beta.1 draft installers carry it. Everything else in the pass was green (export SHAs stable, VM off = byte-identical, cancellation left no partial output). | machine-local `test-output/mac-e2e-3d9ba6c/` (REPORT.md, screenshots, exports; not committed) |
| 2026-09-01 | remediation | `1400373` | — | `src/hooks/useTrackMaster.ts`, `src/lib/settings-transitions.ts` | fix + regression tests | frontend-unit | **FIXED.** `setDeliveryBitDepth`/`setDeliverySampleRate` now route through new `applyDeliveryBitDepthSelection`/`applyDeliverySampleRateSelection`, which run the existing Custom-capture rule (`applyDeliveryProfileSelection(prev, "custom")` — pins effective loudness, ceiling, bit depth, sample rate) before writing the format field. Two regression tests pin the exact repro: a named profile with stale hidden offsets keeps its effective −14.0 LUFS / −1.0 dBTP across a bit-depth-only and a sample-rate-only edit. Reviewed 2026-09-01: the only other direct `delivery_profile: "custom"` write (`standard-export.ts`) explicitly pins all four shadowed fields, and album format choices use the separate album-level mechanism — no remaining bypass. | `src/lib/settings-transitions.test.ts` |
| 2026-09-01 | U14 (re-verification) | `9225da6` (tip: format fix + mobile parity `c40f3a4` + Android bridge packaging `ea1e22b` + browserslist patch `5edddc8`) | macOS 26.5 Apple Silicon / Node 22.23.2, rustup stable cargo | source `0.9.2` | `npm ci` + `npm test` + `npm run build`; `cargo fmt --check`; `cargo clippy --all-targets -- -D warnings`; full `cargo test` **with `AMS_RUN_REAL_FIXTURE=1`**; iPhone bridge `cargo check --all-targets` + `cargo test`; **`xcodebuild test` (iPhone 17 Pro simulator)**; `npm run verify:headless` + `verify:landing-assets` | frontend-unit + native-synthetic + private-fixture + browser-headless | PASS — frontend **831 passed / 3 skipped** (includes the two new format-preservation regression tests); production build clean; fmt/clippy clean; Rust **596 passed / 0 failed, zero fixture skips**, all preset byte-identity snapshots green; iPhone Rust bridge 46 passed / 1 ignored; **iOS simulator XCTests 59 passed / 0 failed — first recorded run of the Swift suite in this ledger (no CI lane compiles Swift), covering the new analysis-presentation hold**; headless landing + `/app` suites green, landing-assets gate green. | run logs (this session) |
| 2026-09-01 | U14 (re-verification) | `9225da6` | GitHub Actions | — | CI run `33539039556` (job-level readback via API) | — | PASS — all **seven** jobs green at the tip: macOS desktop+iPhone, Windows desktop+bridge, Android host JVM, Web E2E headless Chromium, npm+RustSec advisories, snapshot diagnostics (macos, windows). | GitHub Actions run 33539039556 |
| 2026-09-01 | Ship plan S1–S7 (`docs/plans/2026-09-01-friday-ship-execution-plan.md`) | `7d16b06` (final code tip; the S7 records commit on top changes docs only) | Windows 11 office PC / Node + rustup stable | source `0.9.2` | `npm test` + `npm run build` | frontend-unit | PASS — **82 files, 844 tests** (13 new across the run: same-origin head test, five receipt-divert cases, two error mappings, the short-name drift guard, import-label and Settings pins); production build clean; landing-assets gate green after four recaptures of the deterministic set (owner plates untouched). | run logs (this session) |
| 2026-09-01 | Ship plan S3 + S6.5 (D4 window sizing) | `7d16b06` | Windows 11 office PC / rustup stable cargo, `--target-dir target\codex-rc` | source `0.9.2` | `cargo fmt --check`; `cargo clippy --all-targets -- -D warnings`; `cargo test` (default fixture mode) | native-synthetic | PASS — fmt/clippy clean; **611 passed / 0 failed / 7 ignored** across lib + integration suites, including the ten new `window_fit_tests` (maximize decision + native zoom-to-fit); preset byte-identity snapshots green; no DSP file touched. | run logs (this session) |
| 2026-09-01 | Ship plan S2 + S6 (landing head, console surfaces) | `7d16b06` | Windows 11 office PC / Playwright bundled Chromium 149.0.7827.55 | `dist/` from source `0.9.2` | `npm run verify:headless` (cold build + own preview server) | browser-headless | PASS — landing suite green (head URLs from the deployed origin; nav-CTA both-axes and hero-CTA horizontal gates) + **31 `/app` scenario/viewport checks**. Synthetic: proves no native dialog, audio, signing, updater, or listening behavior. | `test-output/headless/2026-09-01T23-45-50-332Z/` (machine-local) |
| 2026-09-01 | Ship plan S3 + S6.5 (packaging) | `7d16b06` | Windows 11 office PC / Tauri CLI 2.11.1, WiX + NSIS | `YES Master_0.9.2_x64_en-US.msi`, `YES Master_0.9.2_x64-setup.exe` | `npm run build:windows` | native-synthetic (dev packaging) | PASS — both bundles produced from the tip with the window-sizing code compiled in. **Not release evidence:** release bytes come only from the tag-triggered Release run (U14/U16). | `src-tauri/target/release/bundle/` (machine-local) |
| 2026-09-01 | Ship plan S7 (Checkpoint 3) | `59d2a0b` | GitHub Actions | — | CI run `33572999889` (job-level readback via `gh run view`) | — | PASS — all **seven** jobs green at the tip: macOS desktop+iPhone, Windows desktop+bridge, Android host JVM, Web E2E headless Chromium, npm+RustSec advisories, snapshot diagnostics (macos, windows). Checkpoint 2 at `0f67d87` was run `33568823137`, also 7/7. The plan's execution record (§11 of the plan) carries the deviation log. | GitHub Actions run 33572999889 |
| 2026-09-04 | D4 startup-fit correction | `e5943f4` | Windows 11 office PC / Node + rustup stable | source `0.9.2` | `npm test`; `npm run build`; `cargo fmt --check`; `cargo clippy --target-dir target\\codex-rc --all-targets -- -D warnings`; `cargo test --lib --target-dir target\\codex-rc`; `cargo test --target-dir target\\codex-rc`; `npm run verify:headless` | frontend-unit + native-synthetic + browser-headless | PASS — frontend **82 files / 844 tests**; production build clean; fmt/clippy clean; Rust full suite green including the client-area regression; landing suite + **31 `/app` scenario/viewport checks** green. Startup zoom now prefers the actual post-maximize webview client area and falls back to monitor work area only when unavailable. | `test-output/headless/2026-09-04T16-44-31-686Z/` (machine-local) |
| 2026-09-04 | D4 display-scaling owner hand-test | `e5943f4` | Windows 11 / 3840×2160 office monitor / installed YES Master 0.9.2 | `YES Master_0.9.2_x64-setup.exe` (SHA-256 `F1FDCA65E69FD8A8651C498D0B7A09F73151222014119EDCEE8FCB2F6EEF51A0`) | launch and inspect Standard + Advanced at 250 % and 300 % Windows display scale | installed-machine | **PASS — owner accepted the result.** At 250 %, Standard is fully visible and Advanced has its intended center/right-rail scrolling. At 300 %, Standard remains fully visible/manipulable after the client-area fit correction; Advanced content remains accessible; rendering is crisp. The accidental 350 % observation is excluded. This closes the Friday-plan scaling hand-test, not the beta.2 candidate install/listening/updater gate. | installed ProductVersion `0.9.2`, commit stamp `e5943f4`; machine-local installer |
...
*2026-09-01 update:* `main` has moved past the tag with owner-directed
remediation and mobile-parity work (through `9225da6`), and the 2026-09-01
Mac QA pass confirmed a second launch blocker **in the tagged `c750da6`
bytes themselves** (format-only edits corrupting the loudness target — see
the 2026-09-01 ledger rows). The beta.1 tag, draft (id 379883047), and
evidence remain frozen and untouched pending the owner's Task-12
disposition; the recommended disposition is reject beta.1 and tag
`v0.9.2-beta.2` at the exact reviewed `main` tip.
