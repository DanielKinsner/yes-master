...
| First codec scope — sequence settled | Standard and Advanced Track first, then Album after agent verification; continuation already authorized. Normal mastering export, no converter/bypass. Quality choices are 320 (default), 256, 192 and 128 kbps; WAV remains the default format | Encoder feasibility, delivered-file checks, Album-specific validation and product-doc updates. |
