...
**Where things stand. C1, C2 and C3 are CLOSED; C4 (U14) has every
mechanical gate green and is waiting on exactly two owner actions:** authorize
the push of the U14 commits (so remote CI can go green at the tip), and
authorize the candidate tag `v0.9.1-beta.1`. See the U14 ledger row above and
the go/no-go "Candidate freeze" section. **The freeze is DECLARED: do not
commit to `main`.** Work discovered from here goes to the owner input queue
or a branch.
...
**After the owner pushes and tags:** C4 is closed and everything remaining is
owner-lane (U15 installed/listening gates → U16 release/updater transaction →
U17 public activation). U15 installs the exact draft artifacts whose SHA-256s
are in the go/no-go ledger — do not rebuild them from a different commit.
