...
> **September 21 launch preparation:** the [current execution record](../reviews/2026-09-21-launch-preparation.md)
> tracks the September 22 target, actual completed local tests and presentation
> repairs. Packaging, remote platform CI and remaining real-machine/release
> gates remain explicit. Resume from this record rather than the old outline.
...
> requesting owner input. Successful checks are recorded; reported findings and
> specific remaining checks need follow-up. Do not treat the session as missing
> or repeat the questionnaire. This plan remains the beta program, the September 5
> plan is the bounded listening follow-up, and `beta-go-no-go.md` remains the
> release gate. No application fixes are marked implemented by this docs update.
...
| U6 | C3 | Complete | `35dc5ad`, `590298d`, `f7e7df5`, `b674c76` | **The page argued in the wrong order.** `CrossPlatform` was the SECOND section — a parked, unobtainable product in the most valuable slot on the page, carrying a phone screenshot and six present-tense feature cards. It is now one date-free sentence low on the page, which is all `docs/landing-brief.md` permits. Page runs one hierarchy: Hero (problem/outcome) → **Workflow** (three decisions, new) → ProofDeck (proof) → **SoundCharacter** (character + adaptive restraint, new) → **AlbumProof** (depth, new) → **BetaTerms** (the arrangement, new) → CrossPlatform → FinalCTA. Album sits low deliberately: album-minded creators are inside the primary audience, not a second one. Audience order is asserted positionally (independent artists → first-few-tracks creators → engineers), with "broke"/"amateur"/"nonprofessional" and every rescue framing banned by test — adaptation is pitched as **restraint** (reduce-only Tier-1 guardrails are what actually ships), never as fixing someone's mix. **Claims executed:** C-05's absolute "true-peak safe, every time" → the fixed 44.1 kHz / 24-bit format and the −1 dBTP ceiling the limiter targets (the product deliberately lets you overcook a track and warns rather than refusing); C-07's hand-authored receipt now carries an **Example receipt** label until U7 binds a real capture; C-15/C-16/C-17 mobile schedule, feature list, and unbound screenshot removed; C-22's "beta testers keep $29 forever" entitlement removed (terms undecided), leaving only the settled $29→$49 one-time model. **8 new claim rows C-27…C-34** added, each with a named evidence source. **17 copy tests**, rendered via `renderToStaticMarkup` so everything asserted is content a visitor gets with no JS and no motion at all. **Verified red four ways** (restoring the schedule language, moving mobile back to second, restoring the entitlement, restoring the absolute guarantee ⇒ 6 tests red). Landing lane gained a `requiresNavAnchor` flag so `#mobile` could lose its nav slot while still being a required section. **FINDING — caught by looking at a phone screenshot, not by a failing test (`590298d`):** the newsletter button read **"Email updates opening soon"** — the banned roadmap word, promising a schedule for a feature with no provider selected — while keeping the full CTA gradient in its disabled state, directly beneath a correctly-inert download button. And its success state promised to **"save your founder price"**, the owner-blocked C-22 entitlement, *pinned by a passing test*: unreachable while unwired, live the moment U4 wires a provider. A test asserting an owner-blocked claim is worse than no test, because it makes the claim look approved. All three fixed; `/\bsoon\b/` now fails the page-level scan. **Also corrected my own evidence:** the first visual pass captured `newPage({ viewportSize })`, which Playwright ignores, so both "desktop" and "phone" runs were 1280×720 — I reviewed a phone layout I had never actually seen. Re-captured at real 390×844 before accepting. **REMAINS FOR THE OWNER:** the taste verdict — does this read *assured, intelligent, alive*, approachable without being cheap. Carried to U15 as a review item, not a work item. |
| U7 | C3 | Complete | `e0e3ad0`, `5877c54` | **The proof on the page was a month stale and nothing could tell.** `desktop-standard-ui.png` and `desktop-advanced-ui.png` were captured **2026-06-28**; U9, U10 and U11 then changed accessible names, warning ownership, the review gate (now a modal), the sticky export cluster and the title clamp. A screenshot has no expiry, so the page quietly advertised a build that no longer exists. **Freshness is now mechanical:** `src/assets/landing/manifest.json` binds every capture to a **capture-input digest** — a content hash over the explicit `CAPTURE_INPUTS` list in `scripts/lib/landing-assets.mjs` (App shell, all 20 non-test components, `preview-mock.ts`, and the capture script itself). **Deliberately NOT `HEAD`:** HEAD advances for docs, Rust and CI changes that cannot affect a screenshot, and a gate that cries wolf on every commit is one people mute — proven by a negative control where a docs edit leaves valid proof valid. A listed input that goes missing throws rather than silently shrinking coverage. **Three captures** (Standard, Advanced, Album-4) replace two, taken from the current build at 1440×1000. **The settle signal had to be corrected:** the headless lane's `~800ms` note and the "No waveform yet." text both flicker; the reliable marker is the playhead `[role="slider"][aria-valuenow]`, which lands ~6s in — without it the marketing images show an empty waveform box, the exact bug U11 shipped in its own evidence. **Eager imagery 2530 KB → 330 KB** against the 1.5 MB budget: captures are `loading="lazy"` with declared intrinsic sizes (no layout shift), the three proof icons were **400×400 PNGs rendered into a 56px box** (→168px, 264 KB → 41 KB), and the hero ships as a 1280w/2560w `srcSet` (1382 KB → 286 KB). No image dependency was added — Chromium is already pinned by the lockfile and canvas resampling is deterministic for a fixed browser build. **Own bug caught and fixed mid-unit:** the first optimizer wrote the recompressed hero back over its own input, so a second run would stack generation loss on the master art while a comment claimed the opposite; the pristine master now lives at `hero-control-room-studio-source.jpg`, is imported by nothing, and reaches no bundle. **A lazy-loading false positive was also found and fixed in the landing lane:** its broken-image check treated `!image.complete` as broken, which is the normal state of a below-fold lazy asset — narrowed to `complete && naturalWidth === 0`, and a new scroll-to-bottom step now *proves* the lazy assets actually arrive (a lazy image that never resolves is a broken image with better manners). **4 negative controls** all behave: stale input ⇒ fail naming the digest; altered bytes ⇒ fail naming hash and size; unrelated commit ⇒ **pass**; shrunken budget ⇒ fail naming the overage. 7 tests in `src/lib/landing-assets.test.ts` run the real gate rather than reimplementing it, plus a CI step ahead of the browser suites so a stale capture fails in seconds. **FINDING for the owner (taste, not mechanics):** the hero photograph is a *rendered illustration* of a mastering UI that does not match the shipped app (its signal-chain labels differ from the real SOURCE/EQ/WARMTH/AIR/COMP/WIDTH/SATURATION/LIMITER row). It is decorative and `aria-hidden`, and the three real captures carry the actual proof, so this is not a false claim — but a reviewer could reasonably want the hero art to depict the real console. Owner call, carried to U15. **Also noted:** `iphone-standard-ui.jpg` (917 KB) is now imported by nothing after U6 and reaches no bundle; left on disk rather than deleted, since removing owner art is not an agent decision. |
| U8 | C3 | Complete | `4dac4bb`, `d024bcb` | **`320×568` added — S-A3 named it and no viewport covered it**, so the scenario could not pass as written; the matrix is now **13 viewports**. Per viewport the lane asserts: both acquisition CTAs fully on screen, no in-page link pointing at a missing section, every external link https with `rel` on `_blank`, exactly one `h1` with no skipped heading levels, a `<main>` landmark, a skip link targeting `#main`, title/description/canonical sanity, and touch targets ≥24px **at phone widths only**. Two deliberate narrowings, both because an over-broad gate gets muted rather than obeyed: the touch rule does not apply at desktop widths (the input is a mouse, and a 20px nav link is not a defect there), and links sitting **inline inside a sentence** are exempt per WCAG 2.5.8 (their size comes from the prose; padding them wrecks the paragraph). **axe-core** (added as a devDependency, injected as a script — no adapter to keep in sync) runs at 1440×900 and 390×844 against `wcag2a/wcag2aa/wcag21a/wcag21aa`, with best-practice rules excluded because they are opinions; **serious and critical fail**, lower impacts are recorded. Two widths not twelve — axe findings are overwhelmingly width-independent, so more runs buy repetition rather than coverage. Result: **28 passes, 0 violations** at both widths. **Keyboard path** proven end to end: first Tab stop is the skip link, it becomes visible and shows a focus indicator, and the CTA is reachable within 40 stops without focus falling to `<body>`. **200% zoom is measured, not promised** — WCAG 1.4.4 is normally discharged as a "recorded manual check" nobody repeats; the lane zooms, re-measures overflow (0 at 1440) and confirms the CTA is still on screen. **Reduced motion** must still contain every load-bearing sentence. **Indexability is tied to release state and fails in BOTH directions**, so U17 cannot forget to remove `noindex` and nobody can remove it early; head metadata (title/description/canonical/OG/Twitter/`og:image` served from `/public`, outside the eager budget) is additionally pinned by 5 browserless tests so a bad `index.html` edit fails in the same second. **Production smoke** (`npm run verify:landing-smoke -- <url>`) adds real outbound-link resolution for U17; off by default because a hermetic lane must not depend on third-party availability. **DEFECT FOUND AND FIXED:** while the download is closed, the *only* next action on the page — "Watch the releases page on GitHub" — was a bare text link under 24px tall, i.e. the hardest thing to tap on the page and the thing a phone visitor most needs. **A second finding was in my own new check:** `ERR_ABORTED` was failing the lane, but that is the browser cancelling a superseded `srcset` candidate on a viewport change — a cancellation is not a broken resource, and treating it as one would have made U7's responsive hero unshippable. **3 negative controls**, all precise: removing an `alt` ⇒ axe *critical* `image-alt` at both widths; removing `noindex` ⇒ FAIL naming the release state; removing the skip link ⇒ FAIL at every viewport. **REMAINS FOR THE OWNER:** a real screen-reader pass (NVDA/VoiceOver) — axe is static analysis and cannot hear anything. Carried to U15. |
| U14 | C4 | Complete for `v0.9.1-beta.1` — **candidate superseded 2026-08-19 (owner); version 0.9.2; gates to re-run and re-tag `v0.9.2-beta.1`, freeze lifted until then** — *[2026-08-31: re-tagged `v0.9.2-beta.1` @ `c750da6`, then audit-blocked pending owner disposition; see the 2026-08-31 addendum below]* | `7b8fc84`, `2955a8a`, `f4fd637`, `f315105`, `b00c4c8`, `48ed322`, `4e9fb3f`, `5c008f6` | **Version 0.9.1 everywhere** (3 manifests, 3 Cargo.locks, preview mock, bug-form placeholder; coherence test green). **New `track_master_e2e.rs`**: one synthetic WAV walks import→analysis→waveform→render→re-analysis→receipt/checks→collision-safe second export→save→reload, plus mono; negative-controlled (wrong divert name ⇒ red). **Every local lane green at `b00c4c8`**: npm test 72/712; audit 0 vulns; full cargo suite **with the real-fixture lane enabled on this machine** — 588 passed, 0 failed, 0 fixture skips; snapshots unchanged; iPhone 46; Android 26 + arm64 ndk check; headless 13 viewports + 24 app checks; asset gate re-captured at 0.9.1 (digest `77e28589b93a…`). **Draft artifacts rebuilt from the remediated tip + SHA-256 recorded** (msi `0ee6d1…`, nsis `7b443c…`), unsigned, unpublished. **Five defects found by first-ever real-Windows/watched-CI runs, all fixed:** host-dependent `detectPlatform(undefined)` test; `URL#pathname` path mangling on a checkout with a space; bare `npm`/POSIX group-kill/`::1`-bind Windows failures in the capture script; **the `lexically_normalize` parity bug** (excess `..` cancelled in pairs at the root — macOS CI red since U12, verdict depended on temp-dir depth parity; fixed `48ed322` with parity-pinned regression tests verified red); and the close-out's freeze wording breaking its own `release-readiness` pin. **The first watched CI run showed main red since U12** — prior sessions pushed and never watched a run finish. Reruns at `4e9fb3f`: cargo 590/0 with fixtures, frontend 712/712, bridges green. **Scenario closure:** S-D1/S-F1 (frontend unit + named headless cases), S-F3 (headless export cases + native E2E) have named mechanical results; S-A3/S-G1 were closed by U8/U4. **Remote CI green at `5c008f6`** (run `30284245225`, all six jobs — first green main run since U12). Tag `v0.9.1-beta.1` on the close-out commit, freeze in force through U17. C4 CLOSED — hard stop; U15–U17 are owner-lane. |
| U15 | C5 | Owner listening evidence recorded 2026-09-05; findings and installed-candidate/platform gates remain | — | Native Windows session on owner-verified `e600a21`; [reconciled results](../listening/2026-09-05-owner-handoff.md) and [targeted follow-up](2026-09-05-listening-follow-up.md). Do not restart the full owner pass. This dev-session evidence does not close the installed-candidate or release gates. |
| U16 | C5 | Not started | — | — |
| U17 | C5 | Not started | — | — |
| U18 | CP | Ready — Mac session required | — | Analyzed + decision-completed 2026-08-25 on the Windows box (Swift cannot compile there; this program does not ship unverified code). Execution plan with verified line anchors, decided gates (`canSelectSide`/`canCreateMaster` semantics), transport design, a11y spec, test list, and lanes: `docs/plans/2026-08-25-u18-iphone-remediation-plan.md`. Both impossible states were confirmed real in current source (Mastered selectable with no track; Create Master tappable with no track, error-driven). |
| U19 | CP | Complete | `bf8e402`, `0f97f0c` | **Every named gap was real.** Both sliders were anonymous to TalkBack (Intensity announced a bare percent, seek a meaningless 0..1 fraction — now named, seek announces "0:32 of 2:10"); the Volume Match switch was a nameless toggle beside an unassociated label (now one merged toggleable row); style-tile selection was color-only (now RadioButton selection semantics, and the grid was de-lazied because its fixed 180 dp clipped row two at large font); Done/Error screens could push their own buttons off-screen with no scroll (now scrollable; the weight spacers they relied on measure to zero under scroll anyway). Interruption notices, the Preparing reason, and the Working stage label are polite live regions. Theme extracted to `ui/theme/Theme.kt` with the policy stated: pinned dark console in BOTH OS modes. **Lane choice (explicit): JVM/Robolectric Compose tests** (14, in `src/testDebug` — the host activity merges into the debug manifest only, release APK untouched) gating the same `gradlew test` CI runs; negative-controlled (removing the Intensity name + receipt scroll ⇒ 5 red). Physical-device TalkBack/background-audio/file-flow proof stays U20. JVM suite green both variants (21 existing + 14 new). |
...
### Session state — 2026-07-27 (HISTORICAL — superseded as resume authority)

> **Do not resume from this block.** It described the `v0.9.1-beta.1`
> candidate, which the owner superseded on 2026-08-19; the current candidate
> and its audit-blocked status live in the **2026-08-31 addendum above**,
> which is the only resume authority. In particular, the U15 instruction
> below to install from the `v0.9.1-beta.1` draft is obsolete: U15 runs only
> against the post-disposition candidate (beta.1 if the owner clears it, or
> the beta.2 rebuilt from the reviewed remediation branch). The C1–C4
> execution history in this block remains accurate as history.
...
time in this program.** That environment shift alone surfaced and fixed four
latent defects (one host-dependent test assertion, three Windows-only script
...
**Where the Rust lane ran.** *(2026-07-25 sessions)* WSL2 checkout driven by
the Windows toolchain (rustc 1.95.0) over `\\wsl.localhost\...`. *(2026-07-27,
U14)* A native Windows NTFS checkout on a machine with the private fixture
folders present — all Rust lanes including the slow real-fixture lane ran
locally with `--target-dir target\codex-rc`.
