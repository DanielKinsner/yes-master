...
| 11 | **Mobile store-readiness** (iPhone PrivacyInfo/real-progress/error-enum; Android signing+keystore; background-audio) | roadmap Wave 8 / S8.x | Keystore-free batch delegated: 2026-07-04 recon verified S8.4's three Android items already shipped (monochrome `ic_launcher.xml:5`, share/play intents `MainActivity.kt:352`, ext check `MasteringViewModel.kt:83`) and drafted the Codex prompt for the iPhone remainder (`docs/plans/2026-07-04-codex-iphone-store-readiness-prompt.md`). Still owner-gated: S8.5 Android signing (keystore), S8.6 background-audio (decision), S8.1 archive validation (Mac). |
