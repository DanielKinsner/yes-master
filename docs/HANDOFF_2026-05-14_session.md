...
> **One-paragraph snapshot.** App renamed Album Mastering Studio → **YES Master** (current HEAD `63f452f`). All seven UI restyle slices + the post-restyle UX restructure landed earlier in the day; Codex then shipped five more commits: a console-layout pass (`a3fcc25`) that locks Track Master to a 5-row CSS grid + adds a real-PCM Original-playback path with matched metering / FFT spectrum, the rename itself (`6a441d9`), a default-window bump to 1920×1080 with minWidth 1440 / minHeight 860 (`1ea2fa5`), a zoom-reset (`4878140`) that forces 100% on every launch and reverts the variable-zoom keybindings from `4f1e53d`, and a Windows-DPI workaround (`a09fe28`) that forces the **physical** window to 1920×1080 and sets WebView zoom to `1 / scale_factor` at runtime so the CSS design canvas stays 1920×1080 on Dan's 150%-scaled Windows display. A second-opinion DSP audit ran against pre-DPI HEAD; verified items are in the new "DSP Debt — Audit findings" section below. The **next workstream is preset character retuning** per `docs/PRESET_REFERENCE_ANALYSIS_2026-05-14.md` — the missing piece that makes presets feel like creative directions instead of tonal cousins. Everything else is queued behind it.
...
- **HEAD**: `63f452f Handoff: slot verified DSP audit findings + Codex's 1ea2fa5/4878140` (this doc's commit, on top of Codex's `a09fe28` DPI fix)
- **Remote**: pushed to `origin/master`
- **Local tree**: clean at session-end
- **Tests** (from session memory, last run before Codex's four post-handoff commits): `cargo test --lib` 80/80; `cargo test` 138/138 with the real-fixture tests skipping by default (set `AMS_RUN_REAL_FIXTURE=1` for the slow lane). `npm run build` clean. **Rerun both as a first move next session.** `a3fcc25` added a new `MeteredPcmSource` constructor signature, `1ea2fa5` rewrote 177 lines of CSS for the higher-resolution canvas, `4878140` simplified the zoom hook, and `a09fe28` added `configure_main_window_physical_target` to `lib.rs` with a runtime DPI workaround — all worth confirming green.
...
| `4878140` | **Codex** — `useWebviewZoomShortcuts` simplified to **force 100% zoom on every launch**. Reverts the variable-zoom behavior shipped at `4f1e53d`; `Ctrl+=` / `Ctrl+-` keybindings are removed. `Ctrl+0` still resets to 100% (effectively a no-op now). Note this is the WebView-side zoom hook only; the runtime DPI workaround in `a09fe28` below sets a `1 / scale_factor` zoom of its own at startup, so on Dan's 150%-scaled machine the actual launch zoom is ~0.667 even though the JS hook reports 1.0. |
| `a09fe28` | **Codex** — **Windows DPI workaround.** Adds `configure_main_window_physical_target` in `src-tauri/src/lib.rs` that, at app setup, reads the window scale factor, forces the **physical** window size to 1920×1080, sets `minSize` to 1440×860 physical, and applies `set_zoom(1.0 / scale_factor)` to the WebView. The `tauri.conf.json` width/height/minWidth/minHeight values dropped to 1280×720 / 960×573 (the **logical-pixel equivalent at 150% scaling**) — those are now just the fallback, the runtime override is what ships. Net effect on Dan's 150%-scaled Windows display: physical window is exactly 1920×1080 px, CSS sees a 1920×1080 design canvas. On a 100%-scaled display it'd be identical. **This commit is why the JSON values "got smaller" — they didn't, the runtime values took over.** Note: Dan can't change Windows scaling (concurrent video editing), so this is the durable solution to the DPI problem the earlier in-app zoom keybindings (`4f1e53d`) tried to solve. |
