...
fn import_one(path_str: &str) -> CommandResult<ImportedTrack> {
    ...
    // Untrusted-parser work stays behind decode's panic boundary — this file
    // owns no Symphonia parsing of its own (2026-08-31 audit, L-01: a local
    // unguarded probe here let a crafted 0 Hz WAV panic the import command).
    let metadata = crate::decode::probe_audio_metadata(path)?;
