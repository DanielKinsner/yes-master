...
#[tokio::test]
async fn analyze_tracks_core_lite_skips_deep_analysis_but_core_includes_it() {
    // Task 9: the mobile/FFI path (`analyze_tracks_core_lite`) gates the Tier-2
    // deep scan OFF to save battery/CPU, while the desktop path
    // (`analyze_tracks_core`) keeps it ON. Both share the same impl so the rest
    // of the AnalysisResult is identical-shape. Use a >=1 s stereo sine
    // (>= SHORT_WINDOW frames) so the deep scan WOULD fire on the core path.
    let tmp = tempfile::tempdir().expect("tempdir");
    let path = tmp.path().join("gate.wav");
    write_sine_wav(&path, 48_000, 1.5, 440.0, 2); // 72000 frames >> SHORT_WINDOW
    let path_str = path.to_string_lossy().to_string();

    let req = engine::AnalyzeRequest {
        id: TrackId("deep-gate".to_string()),
        path: path_str,
    };

    // core (desktop) path -> deep present:
    let core = engine::analyze_tracks_core(vec![req.clone()])
        .await
        .expect("core analyze");
    assert!(
        core[0].deep_analysis.is_some(),
        "desktop core path computes DeepAnalysis"
    );

    // lite (mobile) path -> deep gated OFF, but the rest of the result is
    // identical-shape:
    let lite = engine::analyze_tracks_core_lite(vec![req])
        .await
        .expect("lite analyze");
    assert!(
        lite[0].deep_analysis.is_none(),
        "lite path skips DeepAnalysis"
    );
    // sanity: the wire-relevant measurements still produced on the lite path
    assert!(lite[0].lufs_integrated.is_finite());
}

#[tokio::test]
#[ignore = "manual benchmark: prints deep-scan cost"]
async fn bench_deep_scan_cost_core_vs_lite() {
    use std::time::Instant;
    // ~30 s stereo wav — enough windows to be representative of the deep scan
    // cost we avoid on the mobile path.
    let tmp = tempfile::tempdir().expect("tempdir");
    let path = tmp.path().join("bench.wav");
    write_sine_wav(&path, 48_000, 30.0, 440.0, 2);
    let path_str = path.to_string_lossy().to_string();
    let req = || engine::AnalyzeRequest {
        id: TrackId("bench".to_string()),
        path: path_str.clone(),
    };

    let t0 = Instant::now();
    let core = engine::analyze_tracks_core(vec![req()])
        .await
        .expect("core analyze");
    let core_ms = t0.elapsed().as_secs_f64() * 1000.0;
    assert!(core[0].deep_analysis.is_some());

    let t1 = Instant::now();
    let lite = engine::analyze_tracks_core_lite(vec![req()])
        .await
        .expect("lite analyze");
    let lite_ms = t1.elapsed().as_secs_f64() * 1000.0;
    assert!(lite[0].deep_analysis.is_none());

    eprintln!(
        "BENCH deep-scan cost: core(deep)={core_ms:.1}ms lite(no-deep)={lite_ms:.1}ms \
         delta={:.1}ms (30 s stereo source)",
        core_ms - lite_ms
    );
}
