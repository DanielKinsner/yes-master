...
impl Summary {
    fn new(
        ...
        let mut blocks = Vec::with_capacity(frames.div_ceil(CORE));
        ...
        for start in (0..frames).step_by(CORE) {
            ...
            for frame in start..(start + CORE).min(frames) {
...
pub fn measure(
    ...
    for channel in 0..channels {
        let summary = Summary::new(samples, channels, channel, &cancelled)?;
        ...
        if peak.continuous_upper / grid_denominator > peak.grid_lower * width_ratio {
            let tree = tail_moments::TailTree::new(samples, channels, channel, &cancelled)?;
